...
const char *WINDOWTITLE = { "Project 1 -- Yi-Jung Chiang" };
...
int main(int argc, char *argv[]) {
    ...
    // display structures that will not change
    InitLists();

    // init global vars used by Display() (and post redisplay)
    Reset();
...
//    glTexImage2D( GL_TEXTURE_2D, 0, 3, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, tex0);
    ...
    glLoadIdentity();
    glColor3f(1., 1., 1.);
    DoRasterString(5., 5., 0., "Yi-Jung Chiang - Project 3");
...
void InitMenus() {
    ...
    glutAddMenuEntry("Without texture",             NoTexture);
...
unsigned char* BmpToTexture(char *filename, int *width, int *height) {
    ...
    fp = fopen(filename, "rb");   //path of the file
    if (fp == NULL) {
